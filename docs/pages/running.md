...
```console
% bundle exec rsmp-validator run test/site --site-config config/gem_tlc.yaml --auto-site-config config/simulator/tlc.yaml
```
...
```console
% bundle exec rsmp-validator run test/site/core test/site/tlc/clock_spec.rb --site-config config/gem_tlc.yaml --auto-site-config config/simulator/tlc.yaml
```

You can override the configured Core and SXL versions for a run without editing the YAML config:

```console
% bundle exec rsmp-validator run test/site --site-config config/gem_tlc.yaml --core 3.3.0 --sxls tlc:1.3.0
```

Use a comma-separated `name:version` list to test with several SXLs:

```console
% bundle exec rsmp-validator run test/site --site-config config/gem_tlc.yaml --sxls tlc:1.3.0,vms:1.0.0
```
