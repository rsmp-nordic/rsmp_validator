...
6. The test status is reported back to sus. Sus collects results from all tests and generates a report.
...
You can also use `with_site(:reconnected)` to request that the RSMP connection is closed and reestablished before continuing, or `with_site(:disconnected)` to ensure the connection is closed. See the documentation of the [`with_site`]({{ site.baseurl}}{% link pages/test_site.md %}) helper for details.
...
Often, you will not need to include any exception handling in your test code. If an exception is raised, the test will abort and sus will catch the error and move on to the next test.
...
First `with_site(:connected)` is used to wait for a connection.
...
Note that there is no code for handling exceptions or errors. Any error will cause the test to abort. The error will be caught and recorded by sus.

### Helper methods
Many tests involve similar steps. To avoid duplicating code between tests, the common steps have been factored out as helper methods under `lib/rsmp/validator/helpers/`.
...
The local RSMP supervisor therefore runs inside an Async reactor. Each sus test runs inside an Async task managed by the validator, so test code can use the `rsmp` gem without blocking sus.

It also means that you cannot usually interact with the RSMP site outside the `with_site` block.
...
However, all incoming RSMP messages are checked against the RSMP JSON Schema using the schema validation built into the `rsmp` gem to validate the format, attribute names, etc.
