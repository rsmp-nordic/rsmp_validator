...
describe 'DocGen integration' do
	...
	with 'parsing valid/site/tlc/io_spec.rb' do
		...
		it 'root context is Site::Tlc::Io' do
			skip 'io_spec.rb not found' unless File.exist?(REAL_IO_SPEC)
			expect(contexts.first.name).to be == 'Site::Tlc::Io'
		end

		it 'has an IO subcontext' do
			skip 'io_spec.rb not found' unless File.exist?(REAL_IO_SPEC)
			io = contexts.first.subcontexts.find { |c| c.name == 'IO' }
			expect(io).not.to be == nil
		end

		it 'has Input and Output subcontexts under IO' do
			skip 'io_spec.rb not found' unless File.exist?(REAL_IO_SPEC)
			io = contexts.first.subcontexts.find { |c| c.name == 'IO' }
			names = io.subcontexts.map(&:name)
		...
		it 'extracts spec docstrings' do
			skip 'io_spec.rb not found' unless File.exist?(REAL_IO_SPEC)
			io    = contexts.first.subcontexts.find { |c| c.name == 'IO' }
			input = io.subcontexts.find { |c| c.name == 'Input' }
	...
	with 'rendering valid/site/tlc/io_spec.rb' do
		...
		it 'root page has parent Test Suite' do
			skip 'io_spec.rb not found' unless File.exist?(REAL_IO_SPEC)
			content = File.read(File.join(tmp, 'site_tlc_io.md'))
			expect(content).to be =~ /^parent: Test Suite$/
		end

		it 'depth-1 page has correct parent and grand_parent' do
			skip 'io_spec.rb not found' unless File.exist?(REAL_IO_SPEC)
			content = File.read(File.join(tmp, 'site_tlc_io/io.md'))
			expect(content).to be =~ /^parent: Site::Tlc::Io$/
			expect(content).to be =~ /^grand_parent: Test Suite$/
		end

		it 'depth-2 page has correct parent and grand_parent' do
			skip 'io_spec.rb not found' unless File.exist?(REAL_IO_SPEC)
			content = File.read(File.join(tmp, 'site_tlc_io/io/input.md'))
			expect(content).to be =~ /^parent: IO$/
			expect(content).to be =~ /^grand_parent: Site::Tlc::Io$/
		end
	end
